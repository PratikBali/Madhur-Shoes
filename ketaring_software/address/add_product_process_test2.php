...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Perform additional operations like inserting data into the database
    // Example (insert the barcode):
    $stmt = $conn->prepare("INSERT INTO main_shoes (bar_code_no, ...) VALUES (?, ...)");
    if ($stmt->execute([$bar_code_no])) {
        echo "Barcode successfully generated or added: " . $bar_code_no;
    } else {
        echo "Error adding barcode.";
    }
}
...
if (isset($pdo)) {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO main_shoes_original (bar_code_no, brand_name, product_id, product_seller_name, 
        ...
        $stmt = $pdo->prepare("INSERT INTO main_shoes (bar_code_no, brand_name, product_id, product_seller_name, 
                                category, male_category, female_category, child_male_category, child_female_category, 
                                other_category, artical_no, product_color, product_size, quantity, original_price,
                                sell_price, side, back, up, bottom) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
